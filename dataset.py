...
def draw_circle(img_, pt, r=5):
    pt = pt.astype(int)
    img_ = np.pad(img_, (r, r), constant_values=1)
    pt = np.array([pt[0] + r, pt[1] + r])
    x_axis = np.linspace(-1, 1, r*2)
    y_axis = np.linspace(-1, 1, r*2)
...
class MyAugmentationDataset(data.Dataset):
    def __init__(self, path) -> None:
        super().__init__()
        self.images = [img for img in Path(path, "img").glob("*")]
        self.corner = [cor for cor in Path(path, "label_cor").glob("*")]
        assert len(self.images) == len(self.corner)
        print("Found {} images and {} annotations".format(len(self.images), len(self.corner)))

    def rotate_heatmap(self, degrees, heatmap):
        heatmap = heatmap.permute(1,2,0)
        degrees = (360 + (degrees % 360)) % 360
        value = degrees / 360
        w = heatmap.shape[1]
        offset = int(w * value)
        indices = np.arange(w) + offset
        indices = indices % w
        return heatmap[:, indices].permute(2,0,1)

    def get_img(self, index):
        return load_rgb(self.images[index])

    def get_cor(self, index):
        return np.loadtxt(self.corner[index]).astype(int)
...
class PanoCorBonDataset(data.Dataset):
    ...
    def __init__(self, root_dir,
                 ...
                 normcor=False, return_cor=False, return_path=False, return_mask=False):
    ...
    def __getitem__(self, idx):
        ...
        mask = torch.FloatTensor(mask.copy()).unsqueeze(0)
